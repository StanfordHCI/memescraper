...
class SayingImagesCrawler(BaseCrawler):
    def __init__(self, *args, **kwargs):
        super(SayingImagesCrawler, self).__init__(source='saying_images', *args, **kwargs)
        self.url = 'https://sayingimages.com/category/meme/'
    ...
    def get_galleries(self):
        response = requests.get(self.url + '/', headers={
            'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.14; rv:65.0) Gecko/20100101 Firefox/65.0'})
        soup = BeautifulSoup(response.content, 'html.parser')
        list_items = soup.findAll("div", {"class": "td-module-thumb"})
