...
class FourPMHappyHourCrawler(BaseCrawler):
    def __init__(self, *args, **kwargs):
        super(FourPMHappyHourCrawler, self).__init__(source='four_pm_happy_hour', *args, **kwargs)
        self.url = 'http://4pmhappyhour.com'
        ...
        if response.status_code == 200:
            ...
            for a in posts:
                try:
                    ...
                    for i in imgs:
                        images.append({
                            "id": a.attrs.get('id').strip('post-') + '_' + i.attrs.get('src').split('/')[-1].split('.')[
                                0],
