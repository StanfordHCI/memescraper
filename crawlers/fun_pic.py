...
class FunPicCrawler(BaseCrawler):
    def __init__(self, *args, **kwargs):
        super(FunPicCrawler, self).__init__(source='fun_pic', *args, **kwargs)
        self.url = 'https://www.funpic.us/category/meme/'
    ...
    def get_feed(self, page=1):
        ...
        if response.status_code == 200:
            ...
            for a in posts:
                try:
                    i = a.find('img')
                    img_id = i.attrs.get('src').split('/')[-1].split('.')[0].split('-')[-1]
                    images.append({
                        "id": img_id,
