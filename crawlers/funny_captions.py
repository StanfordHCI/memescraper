...
class FunnyCaptionsCrawler(BaseCrawler):
    def __init__(self, *args, **kwargs):
        super(FunnyCaptionsCrawler, self).__init__(source='funny_captions', *args, **kwargs)
        self.url = 'http://www.funnycaptions.com/tag/memes/'

    def get_feed(self, page=1):
        images = []
        page_url = self.url
        if page > 1:
            page_url = '{}{}/'.format(self.url, page)
        response = requests.get(page_url)
        if response.status_code == 200:
            soup = BeautifulSoup(response.content, 'html.parser')
            posts = soup.findAll('img', {"class": "img_cursor"})
            for i in posts:
                try:
                    img_src = i.attrs.get('src')
                    if 'safe_mode' in img_src:
                        continue
