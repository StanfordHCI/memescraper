...
class ThugLifeMemeCrawler(BaseCrawler):
    def __init__(self, *args, **kwargs):
        super(ThugLifeMemeCrawler, self).__init__(source='thug_life_meme', *args, **kwargs)
        self.url = 'http://thuglifememe.com/'
    ...
    def get_feed(self, g):
        ...
        if response.status_code == 200:
            ...
            posts = soup.findAll('figure', {"class": "wp-block-image"})
            for p in posts:
                try:
                    i = p.find("img")
                    is_post_img = False
    ...
    def get_galleries(self):
        ...
        for li in list_items:
            h2 = li.find("h2", {"class": "entry-title"})
            if h2 is None:
                continue
            yield {
                "id": li.attrs.get("id").strip("post-"),
                "url": h2.find("a").attrs.get("href")
            }
