...
class BoredPandaCrawler(BaseCrawler):
    def __init__(self, *args, **kwargs):
        super(BoredPandaCrawler, self).__init__(source='bored_panda', *args, **kwargs)
        self.url = 'https://www.boredpanda.com/funny/?show=recent'
    ...
    def get_feed(self, g):
        ...
        if response.status_code == 200:
            ...
            posts = soup.findAll('div', {"class": 'shareable-post-image'})
            for p in posts:
                try:
                    i = p.find("img")
                    url = i.attrs.get('src')
